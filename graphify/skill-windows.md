...
- A config entry exists for this repo in `~/.graphify/config.toml` (check via `graphify remote list | Select-String "$(Get-Location)"`).

Otherwise the repo is in **legacy mode** (single `graph.json`). Quick decision tree:

```powershell
if ((Test-Path graphify-out/graph-shared.json) -and (Test-Path graphify-out/graph-private.json)) {
...
Add-Content -Path .graphifyshared -Value "src/**"
Add-Content -Path .graphifyshared -Value "docs/**"
...
If `$HOME\.hermes/state/graphify-sleep.manifest.json` exists in the user's HOME, the sleep cycle is installed. Inspect with:
...
- They run `graphify` against a personal knowledge graph (`$HOME\brain` or equivalent)
...
- Hippocampus = `$HOME\.hermes/MEMORY.md` (small, frozen per session, episodic)
...
```powershell
graphify sleep install --brain-root $HOME\brain
# Paste the printed snippet into the user's Hermes chat session
```

The install command:
1. Copies the skill bundle to `$HOME\.hermes/skills/sleep-cycle/` via `importlib.resources` + `shutil.copytree` (version-aware: re-installs over a different version do a clean rmtree+copy)
2. Writes the cron manifest to `$HOME\.hermes/state/graphify-sleep.manifest.json` with Sprint 1's 2 jobs (Phases 1+2; more jobs added by future `graphify sleep install` after upgrades)
...
4. Reports: `[graphify sleep install] copied skill bundle to $HOME\.hermes/skills/sleep-cycle/ (version 0.10.0-alpha1)`
...
- "Preview the cycle without waiting 24h": `graphify sleep demo --brain-root $HOME\brain`
...
| `$HOME\brain/raw/day_<date>.md` | Phase 1 replay output | NO |
| `$HOME\brain/raw/dreams/<date>.md` | Phase 4 hypothesize justification (Sprint 3) | NO |
| `$HOME\.hermes/state/graphify-sleep.manifest.json` | install manifest | n/a |
| `$HOME\.hermes/skills/sleep-cycle/` | distributed skill bundle | n/a |
| `$HOME\.hermes/cron/output/<job_id>/cost.jsonl` | per-phase LLM cost tracking | n/a |
