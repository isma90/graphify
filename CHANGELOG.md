...
- Feat: `graphify fuse [--batch-size 30] [--max-fusions 50] [--backend X] [--dry-run] [--force-split-mode-unsafe] [<root>]` — KGGen iterative LLM clustering for synonym fusion (Mo et al. arXiv:2502.09956). Batches `degree > 3` candidates, asks LLM to identify clusters of synonym/abbreviation/morphological-variant nodes, builds a fixed-point alias→canonical mapping (chained aliases resolve to terminal canonical, no two-hop staleness), rewrites `G.graph["hyperedges"]` member IDs via the final mapping BEFORE `nx.contracted_nodes` (prevents hyperedge orphaning), applies contractions in-place, strips the `"contraction"` attribute artifact, never fuses nodes with EXTRACTED incident edges (AST identifiers stable; merging breaks source-file traceability). Safety gate: exit 2 above `--max-fusions`.
- Feat: `graphify dream [--threshold 0.7] [--max-triplets 20] [--backend X] [--dry-run] [--force-split-mode-unsafe] [<root>]` — Graphusion novel-triplet inference (Yang et al. arXiv:2407.10794). Identifies the top 5 largest communities with ZERO direct edges between them, samples 3 representatives with `degree > 3` from each, asks an LLM for causal/functional/analogical relations, filters by `--threshold`, drops hallucinated IDs, applies surviving edges with `origin=rem_dream weight=0.5 confidence=INFERRED`. Soft cap `--max-triplets` rolls back excess; hard cap 50 entities triggers exit 2.
- Feat: `graphify hypothesize [--max-hypotheses 5] [--report-path <path>] [--dry-run] [--force-split-mode-unsafe] [<root>]` — Hypothesis nodes from `GRAPH_REPORT.md` Surprising Connections section. Creates nodes with `label="H: <a> ↔ <b>"`, `source_file=dream_log.md`, `origin=hypothesis`, `weight=0.4`, `created_at=now`. Adds DIRECTIONAL `grounded_in` edges (Hypothesis node → Source node). Falls back to `analyze.surprising_connections()` if report format is unrecognized. Idempotent: re-running doesn't create duplicate hypothesis IDs. Writes a markdown justification log to `<brain_root>/raw/dreams/<date>.md`.
- Feat: 4th cron job added to `graphify sleep install` manifest (sleep_4_rem at 03:30 UTC, `context_from=sleep_3_prune`, budget $5.00). Re-balanced budget ratios for total $8.30/night. Re-running install rewrites the manifest with the new job; re-paste the printed Hermes snippet to register.
- Feat: new `templates/sleep_4_rem.md` template in the sleep-cycle bundle. Dispatches 3 `delegate_task` subagents in parallel (fuse + dream + hypothesize), enforces per-subagent safety gates, computes rollback distance from the commits-this-night counter, uses `git reset HEAD~<n>` (NOT `git reset --hard`) on any subagent failure. REM REQUIRES NREM (Phase 2) — skips on NREM failure per CLS dependency rules.
- Tests: +35 net (1507 passing total; same 4 pre-existing fails). New: `tests/test_fuse.py` (8: dry-run, EXTRACTED protection, safety gate, chained mapping fixed-point, working-tree-safe commit, split-mode refusal + force, missing graph), `tests/test_fuse_hyperedges.py` (4: member ID rewriting, deduplication after collapse, chained alias in hyperedge resolves, both `"nodes"` and `"members"` key formats), `tests/test_dream.py` (12: community-pair detection, threshold filtering, hallucinated ID drop, cap rollback, safety gate >50, dry-run, split-mode, missing graph, not-enough-communities exit-clean), `tests/test_hypothesize.py` (12: node creation, directional `grounded_in` edges via `nx.DiGraph` cast, max-hypotheses cap, idempotency, dream log written, missing report graceful exit 0, `surprising_connections` fallback, dry-run, split-mode + force, working-tree-safe commit, Hypothesis node attrs).
- Engineering fix: `graphify hypothesize` casts `nx.Graph` → `nx.DiGraph` after `_load_graph` to preserve edge direction for `grounded_in` (undirected graphs lose insertion order during `node_link_data` round-trip). Surprising-connections section parsing uses a line-by-line state machine instead of `re.DOTALL` regex (the regex's non-greedy `.*?` was always matching empty body).
